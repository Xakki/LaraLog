...
> **LaraLog implementation:** `ExtraProcessor` (`src/Processor/ExtraProcessor.php`) + `BaseContext` (`src/BaseContext.php`). Currently `request_id` goes into `extra` — this is legacy; migration into `context` is planned (see §4.3).
